  ...
  <v-dialog v-model="blockDialog" persistent max-width="290">
    <v-card>
      <v-card-title class="text-h5"
        >Block {{ firstname }} {{ lastname }}?</v-card-title
      >
      <v-card-text class="noBorder"
        >They will not be able to view your profile or posts. They will also not
        be able to follow you.</v-card-text
      >
      <div class="buttons" style="display: flex; justify-content: center">
        <v-btn
          class="block"
          @click="
            blockUser(currentUID, profileUID);
            blockDialog = false;
          "
        >
          Block</v-btn
        >
        <v-btn class="cancel ml-2 mb-2" @click="blockDialog = false"
          >Cancel</v-btn
        >
      </div>
    </v-card>
            ...
            <v-list>
              <v-list-item @click="blockDialog = true">
                <div
                  ...
                  style="color: rgb(180, 25, 25)"
                >
                  <v-list-item-title>Block User</v-list-item-title>
                  <v-icon class="ml-2" style="transform: scale(0.8)"
      ...
      <v-col md="12">
        ...
        This button will not display if a user is looking at their own profile-->
          <v-btn
            ...
            class="mb-4"
            v-if="!isFollowing"
            @click="followUser(currentUID, profileUID)"
            >Follow</v-btn
          >
          <v-btn
            color="login"
            class="mb-4"
            v-else
            @click="unfollowUser(currentUID, profileUID)"
            >Unfollow</v-btn
          >
        </div>
            ...
            <v-list>
              <v-list-item v-for="(userClass, index) in classes" :key="index">
                <v-btn
                  ...
                  :to="'/classes/' + userClass.prefix + userClass.number"
                  >{{ userClass.prefix }}{{ userClass.number }}
                </v-btn>
              </v-list-item>
            </v-list>
          </v-card-text>
...
export default {
  ...
  setup() {
    ...
    const isFollowing = computed(() =>
      followers.value.includes(currentUID.value)
    );
    return {
      firstname,
      lastname,
  ...
  methods: {
    ...
    async followUser(currentUID, profileUID) {
      ...
      try {
        ...
        const profileDocRef = doc(db, "users", profileUID);
